...
| 5+    | Audio data routing (AudioWorklet + offscreen MediaStream) | 🟡 ~70% (TabStreamApi port + adapter 配置完了)       |
...
#### Phase 5+ Step 2b-2a: TabStreamApi port + adapter (✅ 完了, IMPL-611, 本 PR)
...
- 本 PR 時点では offscreen-audio-host からは呼ばれない (Step 2b-2b で配線)
