...
| 0.5.0      | 2026-04-24 | Codex  | Issue #126 Bookmarks 対応: DB-002 `transcript_segments` に `isBookmarked` (boolean, default false) カラムを追加。IndexedDB v4 migration (既存 v1-v3 データは false で埋める)。TranscriptStreamRepository に `toggleBookmark` / `findBookmarked` を追加 |
