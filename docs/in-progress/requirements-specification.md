...
| エンドポインティング方針 | STT プロバイダが文末と判定する無音長・句読点感度・最小発話長の束。息継ぎで文が分断されるのを抑制するための調整軸 |
| 翻訳文脈窓               | 翻訳時に直前 N 個の確定字幕を文脈として渡すためのポリシー。文境界が分断された場合でも訳質を保つための補助機構    |
...
#### 8.2.1 セグメント連続性要件 {#セグメント連続性要件}

動画・会議音声では話者の息継ぎ (200〜800ms) によって STT が文末を誤判定し、
1 発話が複数セグメントに分断される。本要件はその際の字幕・翻訳品質を担保する。

| ID                       | 要件                       | 詳細                                                                                                                                        |
| ------------------------ | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-NF-018 {#req-nf-018} | エンドポインティング調整性 | 利用者はソース単位で無音しきい値・句読点感度・最小発話長を調整可能であること。既定値はプロファイル単位に保持すること                        |
| REQ-NF-019 {#req-nf-019} | 翻訳文脈保持               | 翻訳プロバイダに対して直前 N 個 (既定 3、上限 5) の確定字幕を文脈として送信できる機構を備えること。非対応プロバイダでは安全に無視されること |
...
| RISK-008 | 話者の息継ぎや短い間で STT が文末を誤判定し、字幕・翻訳が途中で分断されて訳質が劣化する | 中     | 高       | REQ-NF-018 のエンドポインティング調整と REQ-NF-019 の翻訳文脈保持で緩和する。LLM 系翻訳プロバイダ導入時に文脈効果を検証する |
